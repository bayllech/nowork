    ...
    <main class="grid gap-8 md:grid-cols-[1.5fr_1fr] md:items-stretch">
      <section class="space-y-6 rounded-3xl bg-white/90 px-8 pb-0 pt-8 shadow-card backdrop-blur">
        ...
        <div class="grid gap-4 rounded-3xl border border-dashed border-primary-light bg-primary-light/40 px-6 pb-6 pt-6">
          ...
          <div class="rounded-3xl border border-white/40 bg-white/60 p-4 shadow-inner">
            <RankingBarChart :items="provinceBarData" title="省份热度 Top 10" color="#5d5bff" unit="次" />
          </div>
          <div class="flex flex-wrap items-center gap-2 text-xs">
            ...
            <span class="text-xs text-muted"><i class="fa-solid fa-circle-exclamation mr-1"></i> 地图为交互组件，支持缩放与提示框</span>
          </div>
        </div>

        <section class="rounded-3xl bg-white/90 px-6 pb-6 pt-4 text-sm leading-relaxed text-muted shadow-card">
          <h3 class="mb-3 flex items-center gap-2 text-base font-semibold text-primary-strong">
            <i class="fa-solid fa-chart-line"></i>
            统计提示
          </h3>
          <ul class="space-y-2">
            <li>• 基于匿名 IP 定位至省级区域，仅用于统计趋势，不采集个人信息。</li>
            <li>• 数据侧重真实愤怒释放场景，图表仅作情绪温度参考。</li>
          </ul>
        </section>
      </section>
      ...
      <aside class="flex flex-col rounded-3xl bg-white/90 p-8 shadow-card backdrop-blur">
        <header class="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p class="text-sm text-muted">综合省份核心城市的怒气指数，展示最新敲击热力 Top 6。</p>
          </div>
        ...
        <p class="mt-4 text-xs leading-relaxed text-muted">
          数据按{{ stats.chinaPeriod === 'daily' ? '今日' : '累计' }}敲击量排序，每 15 分钟刷新一次；若数据相近，优先展示怒气等级更高的地区。
        </p>
      </aside>
    </main>
  </div>
...
const provinceLeaders = computed(() => {
  return stats.chinaProvinces.slice(0, 6).map((province, index) => {
...
const provinceMapData = computed(() =>
  stats.chinaProvinces
    .filter((province) => province.province && province.province !== '未知地区')
    .map((province) => ({
